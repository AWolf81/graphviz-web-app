  ...
  <div>
    ...
    >
      ...
      >
        ...
        <span class="hidden-xs">Settings <span class="caret"></span></span>
      </button>
      <div
        class="dropdown-menu"
      ...
      >
        <div class="form-group">
          <label label-for="scale-input"
            >Scale <small>(only raster image)</small></label
          >
          <input
            id="scale-input"
...
export default {
  ...
  methods: {
    createExportSvg() {
      ...
      svg.setAttribute(
        "viewBox",
        `0 0 ${this.scale * graphBox.width} ${this.scale * graphBox.height}`
      );
    ...
    scaleChanged: _.debounce(function() {
      ...
      this.$ls.set("settings", {
        // todo add object assign later for more settings
        scale: this.scale
      });
    }, 400),
    save(cb) {
    ...
    savePNG() {
      this.save(svg => saveSvgAsPng(svg, this.name + ".png"));
    },
    saveSVG() {
      this.save(() =>
