  ...
  <div class="container-fluid">
    <div class="row" v-if="user">
      <div class="well">
        ...
        <h2>Cloud graphs</h2>
        <div v-if="loadingDotfiles">loading...</div>
        <div v-else>
          <div class="list-group" v-if="dotfiles && dotfiles.length > 0">
            <router-link
              :to="`/${dot.metadata.ghUser}/${dot.slug}`"
              class="list-group-item"
              v-for="dot in dotfiles"
              :key="dot['_id']"
            >
              <h3>{{ dot.title }}</h3>
              <p>
                <span v-html="dot.content"></span>last modified:
                {{ formatDate(dot.modified_at) }}
              </p>
              <button
                class="btn btn-danger"
                @click.prevent="showDeleteConfirm(dot, true)"
              >
                Delete
              </button>
              Visibility:
              <visibility
                :visibility="dot.metadata.visibility"
                @toggle="
                  toggleVisibility(dot.metadata.visibility, {
                    user: dot.metadata.ghUser,
                    slug: dot.slug
                  })
                "
              />
          ...
          <div class="alert alert-info" v-else>No graph saved yet.</div>
        </div>
        <h2>Local graphs</h2>
        <div class="list-group" v-if="storedGraphs.length > 0">
          <router-link
            class="list-group-item"
            :to="`/${graph.slug}`"
            v-for="(graph, key, index) in storedGraphs"
            :key="index"
          >
            <h3>{{ graph.name }}</h3>
            ...
            <button
              class="btn btn-danger"
              @click.prevent="showDeleteConfirm(graph)"
            >
              Delete
            </button>
          </router-link>
    ...
    <modal :show="showDelete" @cancel="cancelDelete" @ok="deleteGraph">
      ...
      <p slot="body">
        ...
        <strong>{{
          graphToDelete && (graphToDelete.name || graphToDelete.title)
        }}</strong
        >? (no undo possible)
      </p>
    </modal>
...
export default {
  ...
  computed: {
    ...
    ...mapState(["showDelete", "graphToDelete", "storedGraphs"])
  },
  data() {
    return {
      dotfiles: undefined,
  ...
  methods: {
    ...
    deleteGraph() {
      this.$store
        .dispatch("triggerRemoveGraph")
        .then(() => {
          if (this.graphToDelete.inCloud) {
            const dot = this.dotfiles.filter(
              dot => dot._id === this.graphToDelete._id
            )[0];
    ...
    toggleVisibility(visibility, params) {
      ...
      const getVisibility = () =>
        this.dotfiles.filter(dot => dot.slug === params.slug)[0];
